            ...
            foreach (var record in slot.content)
            {
                foreach (var fakeInputWithFipAndActions in record.fakeInputsWithFipsAndActions)
                {
                    foreach (var fipAndActions in fakeInputWithFipAndActions.fipsAndActions)
                    {
                        ...
                        result.Add(record.recordName, new Dictionary<FakeInput, Dictionary<object, AtfActionRleQueue>>());
                        result[record.recordName].Add(fakeInputWithFipAndActions.fakeInput, new Dictionary<object, AtfActionRleQueue>());
                        result[record.recordName][fakeInputWithFipAndActions.fakeInput].Add(ParseFip(fipAndActions.fakeInputParameter), newRleQueue);
                    }
                }
            }
            return result;
